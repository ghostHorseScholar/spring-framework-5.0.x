...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 指定用于创建bean实例的回调，作为声明式指定的工厂方法的替代方案。
	 * 如果设置了这样的回调函数，它将覆盖任何其他构造函数或工厂方法元数据。然而，bean属性填充和潜在的注释驱动注入仍然会像往常一样适用。
	 *
	 ...
	 * @since 5.0
	 * @see #setConstructorArgumentValues(ConstructorArgumentValues)  设置构造函数的参数值
	 * @see #setPropertyValues(MutablePropertyValues)  设置属性值
	 */
	 ...
	 * @throws BeanDefinitionValidationException in case of validation failure 以防验证失败
	 * 验证并准备为这个bean定义的方法覆盖。检查具有指定名称的方法是否存在。
	 */
