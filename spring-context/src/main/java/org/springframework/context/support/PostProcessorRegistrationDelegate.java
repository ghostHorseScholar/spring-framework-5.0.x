...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 不要在这里初始化factorybean:我们需要保持所有常规bean不初始化，让bean工厂后处理器应用于它们!
			// Separate between BeanDefinitionRegistryPostProcessors that implement
			// PriorityOrdered, Ordered, and the rest.
			// 独立于实现PriorityOrdered、Ordered和其他的BeanDefinitionRegistryPostProcessors。
		...
		// 清除缓存的合并bean定义，因为后处理器可能修改了原始的元数据，例如替换值中的占位符……
	...
	public static void registerBeanPostProcessors(
		...
		// 注册BeanPostProcessorChecker，它在BeanPostProcessor实例化过程中创建bean时记录一条信息消息，即当bean没有资格被所有BeanPostProcessor处理时。
		...
		// 将检测内部bean的post-processor重新注册为applicationlistener，将其移到处理器链的末端(用于拾取代理等)。
